...
#[no_mangle]
pub fn init() {
    if !Val::is_void(sdk::get_contract_data(ACCOUNTS_FUNDED_KEY)) {
        return;
    }
    let m: Map<Object, bool> = Map::new();
    sdk::put_contract_data(ACCOUNTS_FUNDED_KEY, m.into());
}
...
pub fn fund(acc: Val) -> Val {
    let mut accounts: Map<Object, bool> =
        Map::try_from(sdk::get_contract_data(ACCOUNTS_FUNDED_KEY).as_object()).or_abort();
    accounts = accounts.put(acc.as_object(), true);
    sdk::put_contract_data(ACCOUNTS_FUNDED_KEY, Val::from(accounts));
    sdk::account_create(Val::from_void(), acc, Val::from_u63(1000))
}
...
pub fn trust_asset(acc: Val, asset: Val) -> Val {
    sdk::account_create_trustline(acc, asset)
}
...
pub fn balance(acc_id: Val, asset: Val) -> Val {
    sdk::account_get_balance(acc_id, asset)
}
